...
export function useRating() {
  ...
  const handleRatingWeightsSave = (weights, setRatingWeights) => {
    console.log("handleRatingWeightsSave вызван с весами:", weights);
  ...
  const recalculateAllRatings = (items, weights) => {
    console.log("recalculateAllRatings вызван с элементами:", items);
    const updatedItems = items.map((item) =>
      calculateRating(item, items, weights)
    );
    console.log("Элементы с пересчитанными рейтингами:", updatedItems);
    return updatedItems;
  };

  const createItem = (itemData, allItems, weights) => {
    console.log("createItem вызван с данными:", itemData);
    const newItem = {
      ...itemData,
    ...
    console.log("Обновленный список элементов:", updatedItems);
    ...
    console.log("Элементы с пересчитанными рейтингами:", itemsWithRatings);
    return { newItem, itemsWithRatings };
  };

  const calculateRating = (itemData, allItems, weights) => {
    console.log("calculateRating вызван с данными:", itemData);
    ...
    if (allItems.length > 0) {
      ...
      const totalProsScore = allItems.reduce(
        (sum, item) =>
          sum + (item.pros || []).reduce((s, pro) => s + (pro.impact || 5), 0),
        0
      );
      const totalConsScore = allItems.reduce(
        (sum, item) =>
          sum + (item.cons || []).reduce((s, con) => s + (con.impact || 5), 0),
        0
      );
      ...
      const maxProsScore = Math.max(
        ...allItems.map((item) =>
          (item.pros || []).reduce((s, pro) => s + (pro.impact || 5), 0)
        )
      );
      const maxConsScore = Math.max(
        ...allItems.map((item) =>
          (item.cons || []).reduce((s, con) => s + (con.impact || 5), 0)
        )
      );
      ...
      prosConsImpact =
        (normalizedPros * prosBonus - normalizedCons * consBonus) *
        RATING_CONFIG.PROS_CONS_WEIGHT *
        0.15;
    }
    ...
    const finalRating = Math.max(
      0,
      Math.min(
        RATING_CONFIG.MAX_RATING,
        baseRating + priceImpact + prosConsImpact
      )
    );

    console.log("Рассчитанный рейтинг:", finalRating);
    return { ...itemData, rating: Math.round(finalRating * 10) / 10 };
  };

  const updateItem = (id, data, allItems, weights) => {
    console.log("updateItem вызван с id:", id, "и данными:", data);
    // Обновляем элемент
  ...
  const deleteItem = (id, allItems) => {
    console.log("deleteItem вызван с id:", id);
    const filteredItems = allItems.filter((item) => item.id !== id);
    console.log("Элементы после удаления:", filteredItems);
