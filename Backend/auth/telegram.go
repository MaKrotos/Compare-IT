...
func extractUserData(parsedData url.Values) (*TelegramWebAppData, error) {
	...
	// Парсим данные пользователя из JSON строки user (как в Prophecy)
	if userStr := parsedData.Get("user"); userStr != "" {
		...
		// Если JSON строки user нет, пытаемся получить данные напрямую
		if idStr := parsedData.Get("id"); idStr != "" {
			if id, err := strconv.ParseInt(idStr, 10, 64); err == nil {
				userData.ID = id
			}
		}
		
		if firstName := parsedData.Get("first_name"); firstName != "" {
			userData.FirstName = firstName
		}
		
		if lastName := parsedData.Get("last_name"); lastName != "" {
			userData.LastName = lastName
		}
		
		if username := parsedData.Get("username"); username != "" {
			userData.Username = username
		}
		
		if photoURL := parsedData.Get("photo_url"); photoURL != "" {
